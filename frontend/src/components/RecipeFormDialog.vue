  ...
  const payload = {
    ...
    ingredients: form.value.ingredients
      ...
      .map((i) => ({
        name: i.name.trim(),
        quantity:
          i.quantity === "" || i.quantity === null ? null : Number(i.quantity),
        unit: i.unit || "",
        aisle: i.aisle || "",
      })),
  ...
  >
    <v-card flat>
      <!-- Icon-only close + a title that shrinks with text-truncate, rather than
        ...
        <v-btn variant="text" size="small" :loading="saving" @click="submit"
        >OK</v-btn
        >
      </v-card-title>
      ...
      <v-card-text class="pa-4">
        <div class="d-flex align-center ga-2 mb-4">
          <v-icon size="20" class="text-primary"
            >mdi-silverware-fork-knife</v-icon>
          <span class="text-subtitle-1 font-weight-bold">Recette</span>
          <v-divider class="flex-grow-1 ms-3" />
        </div>
        <v-text-field
          v-model="form.name"
          label="Nom"
        ...
        <div class="d-flex align-center ga-2 mb-4">
          ...
          >
          <span class="text-subtitle-1 font-weight-bold">Ingrédients</span>
          <v-divider class="flex-grow-1 ms-3" />
        </div>
        ...
        >
          ...
          >
            <span
              :class="ingredient.aisleOverridden ? 'em-muted' : 'text-info'"
            >
        ...
        <div class="d-flex align-center ga-2 mb-4">
          ...
          <span class="text-subtitle-1 font-weight-bold">Étapes</span>
          <v-divider class="flex-grow-1 ms-3" />
        </div>
        <div
        ...
        >
          <span class="em-muted em-mono mt-3" style="min-width: 1rem">{{
            index + 1
          }}</span>
          <v-textarea
